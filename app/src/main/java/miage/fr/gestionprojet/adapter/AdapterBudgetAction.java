...
public class AdapterBudgetAction extends AbstractAdapterBudget<String> {

    public AdapterBudgetAction(ActivityBudget context, int resource, List<String> objects) {
    ...
    protected void chargerNbAction() {
        ...
        results = DaoAction.getBudgetTotalByActionTotalGroupByActions();
        if (results.size() > 0) {
            for (String t : getListBudget()) {
                if (results.get(t) != null) {
